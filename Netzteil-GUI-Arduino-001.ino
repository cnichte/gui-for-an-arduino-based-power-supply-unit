...
/* ************************************************
   ...
   https://techtutorialsx.com/2017/10/13/esp32-esp8366-arduino-creating-a-simple-class/
 * *********************************************** */
class Max_Hardware {
  private:
...
class Max_Button {
  public:
    ...
    */
    Max_Button() {
      // Structs have to be initialised in the contructor.
...
class Max_LayoutManager {
  public:
    ...
    // https://github.com/SloCompTech/QList/blob/master/examples/SimpleList/SimpleList.ino
    ...
    void rasterize() {
      ...
      for (int i = 0; i < layout_Items.size(); i++) {
        ...
        // Wo soll der Button im Raster erscheinen?
        // Richte den Button am Raster aus
        ...
        if (layout_Items.at(i).fill_x) {
          // Berecne die Breite des Buttons...
...
class Max_Screen {
  public:
    ...
    /*
       ...
       Kopfzeile, alle Elemente und die Fußzeile.
    */
    void drawScreen() {
      myGLCD.clrScr();
      // myGLCD.fillScr(backgroundFarbe.R, backgroundFarbe.G, backgroundFarbe.B);

      // Rendert alle Buttons auf den Bildschirm
      ...
      myGLCD.setColor(255, 255, 255); // Farbe Schrift weiss
      myGLCD.setFont(BigFont);
      ...
      // Linien zeichnen
      myGLCD.setColor(255, 0, 0); // Farbe rot
...
void performn_Signal() {
  ...
  // ...
  // Auf dem Touch wird rumgedrückt
  if (myTouch.dataAvailable()) {

    // Touch-Koordinaten ermitteln
    myTouch.read();
    x = myTouch.getX();
    y = myTouch.getY();
